#!/usr/bin/env python3
"""Cohort-level NMF deconvolution with atlas anchor, sweep over anchor_weight.
...
from tapestry.benchmark.nmf import run_nmf_deconvolution
...
def main():
    ...
    parser.add_argument("--anchor-weights", type=float, nargs="+",
    ...
    rows = [{"method": "NNLS", "anchor": float("nan"), "s_drift": float("nan"),
             **metrics(nnls_preds, true, ct_cols)}]

    for lam in args.anchor_weights:
        logger.info("Running NMF with anchor_weight=%.2f ...", lam)
        W, S = run_nmf_deconvolution(X, coverage, ref, anchor_weight=lam, max_iter=args.max_iter)
        drift = float(np.linalg.norm(S - ref) / (np.linalg.norm(ref) + 1e-12))
        rows.append({"method": "NMF", "anchor": lam, "s_drift": drift, **metrics(W, true, ct_cols)})

    print(f"\n{'method':6s} {'anchor':>10s} {'S_drift':>8s}  {'MAE':>8s}  {'log_R²':>8s}  {'slope':>7s}  "
          f"{'OAC R²':>8s}  {'OAC MAE':>8s}  {'pres_F1':>8s}")
    print("-" * 95)
    for r in rows:
        a = f"{r['anchor']:.2f}" if not np.isnan(r['anchor']) else "       —"
        d = f"{r['s_drift']:.3f}" if not np.isnan(r['s_drift']) else "      —"
        print(
            f"{r['method']:6s} {a:>10s} {d:>8s}  "
