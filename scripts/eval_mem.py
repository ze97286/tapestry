#!/usr/bin/env python3
"""Maximum Entropy Method (MEM) deconvolution on the eval set.

Supports a sweep over ``lambda_reg`` in a single run — each value is treated
as a separate "method" in the output table. NNLS is always included as the
reference row.
...
from tapestry.benchmark.mem import run_mem_deconvolution
...
def main():
    ...
    rows = [{"method": "NNLS", "lambda": float("nan"), **nnls_m}]

    for lam in args.lambdas:
        logger.info("Running MEM with lambda=%.4f ...", lam)
        preds = run_mem_deconvolution(X, coverage, ref, lambda_reg=lam, max_iter=args.max_iter)
        m = metrics(preds, true, ct_cols)
        rows.append({"method": "MEM", "lambda": lam, **m})

    print(f"\n{'method':6s} {'lambda':>10s}  {'MAE':>8s}  {'log_R²':>8s}  {'slope':>7s}  "
          f"{'OAC R²':>8s}  {'OAC MAE':>8s}  {'pres_F1':>8s}")
    print("-" * 80)
    for r in rows:
        lam = f"{r['lambda']:.4f}" if not np.isnan(r['lambda']) else "       —"
        print(
            f"{r['method']:6s} {lam:>10s}  "
