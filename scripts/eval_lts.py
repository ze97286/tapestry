#!/usr/bin/env python3
"""Least Trimmed Squares deconvolution on the eval set, with trim-frac sweep.
...
from tapestry.benchmark.lts import run_lts_deconvolution
...
def main():
    ...
    parser.add_argument("--trim-fracs", type=float, nargs="+",
                        default=[0.05, 0.10, 0.15, 0.20, 0.30])
    parser.add_argument("--n-starts", type=int, default=3)
    parser.add_argument("--max-c-steps", type=int, default=10)
    ...
    rows = [{"method": "NNLS", "trim": float("nan"), **metrics(nnls_preds, true, ct_cols)}]

    for trim in args.trim_fracs:
        logger.info("Running LTS with trim_frac=%.2f (n_starts=%d)...", trim, args.n_starts)
        preds = run_lts_deconvolution(
            X, coverage, ref,
            trim_frac=trim,
            max_c_steps=args.max_c_steps,
            n_starts=args.n_starts,
        )
        rows.append({"method": "LTS", "trim": trim, **metrics(preds, true, ct_cols)})

    print(f"\n{'method':6s} {'trim':>8s}  {'MAE':>8s}  {'log_R²':>8s}  {'slope':>7s}  "
          f"{'OAC R²':>8s}  {'OAC MAE':>8s}  {'pres_F1':>8s}")
    print("-" * 80)
    for r in rows:
        tr = f"{r['trim']:.4f}" if not np.isnan(r['trim']) else "       —"
        print(
            f"{r['method']:6s} {tr:>8s}  "
