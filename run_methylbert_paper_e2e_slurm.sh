...
n_bulk=0
if [ -n "${METHYLBERT_BULK_BAM_LIST:-}" ] && [ -s "${METHYLBERT_BULK_BAM_LIST}" ]; then
...
if [ "${n_bulk}" -gt 0 ]; then
    ...
    echo "Skipping deconvolution and collect jobs: no BAMs found in METHYLBERT_BULK_BAM_LIST=${METHYLBERT_BULK_BAM_LIST:-unset}" >&2
fi
